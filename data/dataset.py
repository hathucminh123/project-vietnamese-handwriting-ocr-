...
class VietOCR(Dataset):
    def __init__(self, image_path, image_list, label_path, img_w, img_h, phase):
        ...
        self.all_labels = json.load(open(label_path, encoding="utf8")) if label_path != None else None
        self.img_w = img_w
        self.img_h = img_h
        self.phase = phase
        ...
        if self.phase == 'train':
            self.transform = A.Compose([
                ...
                A.CoarseDropout(max_holes=15, max_height=20, max_width=20),
...
def my_collate_fn(batch):
    ...
    all_labels = pad_sequence([label.clone().detach() for label in labels],batch_first=True)
